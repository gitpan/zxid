...
/* FUNC(zx_DEEP_CLONE_a_ProblemIRI) */
...
{
  x = (struct zx_a_ProblemIRI_s*)zx_clone_elem_common(c, &x->gg, sizeof(struct zx_a_ProblemIRI_s), dup_strs);
  /* *** deal with xmlns specifications in exc c14n way */
...
/* FUNC(zx_NEW_a_ProblemIRI) */

/* Trivial allocator/constructor for the datatype. */

struct zx_a_ProblemIRI_s* zx_NEW_a_ProblemIRI(struct zx_ctx* c)
{
  struct zx_a_ProblemIRI_s* x = ZX_ZALLOC(c, struct zx_a_ProblemIRI_s);
  x->gg.g.tok = zx_a_ProblemIRI_ELEM;
  return x;
}

/* FUNC(zx_WALK_SO_a_ProblemIRI) */
